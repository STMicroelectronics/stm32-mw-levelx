...
UINT stm32_lx_w25q128j_nor_flash_polling_driver_initialize(LX_NOR_FLASH *nor_flash)
{
  ...
  w25q128j_status = w25q128j_get_info(p_w25q128j_obj, &nor_flash_info);
  if( w25q128j_status != W25Q128J_OK)
  {
    return LX_ERROR;
  }
                                           ...
                                           ULONG words)
{
  ...
  else
  {
    ...
    status = w25q128j_read(p_w25q128j_obj, (uint8_t *) destination, (uint32_t ) flash_address, words * sizeof(ULONG));
...
static UINT stm32_lx_nor_flash_driver_write(LX_NOR_FLASH *nor_flash, ULONG *flash_address, ULONG *source, ULONG words)
{
  ...
  else
  {
    ...
    status = w25q128j_write(p_w25q128j_obj, (uint8_t *) source,
...
static UINT stm32_lx_nor_flash_driver_block_erased_verify(LX_NOR_FLASH *nor_flash, ULONG block)
{
  ...
  {
    status = w25q128j_read(p_w25q128j_obj, (uint8_t *)nor_ctx->nor_flash_read_buffer, (uint32_t)start_address, LX_NOR_SECTOR_SIZE * sizeof(ULONG));
    if (status != W25Q128J_OK)
    {
      return LX_ERROR;
    }
