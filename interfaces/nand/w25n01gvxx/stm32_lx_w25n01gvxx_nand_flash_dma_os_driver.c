...
static void w25n01gvxx_read_cplt_callback(w25n01gvxx_obj_t *pobj, void *arg);
static void w25n01gvxx_write_cplt_callback(w25n01gvxx_obj_t *pobj, void *arg);
static void w25n01gvxx_spare_read_cplt_callback(w25n01gvxx_obj_t *pobj, void *arg);
static void w25n01gvxx_spare_write_cplt_callback(w25n01gvxx_obj_t *pobj, void *arg);
...
UINT stm32_lx_w25n01gvxx_nand_flash_dma_os_driver_initialize(LX_NAND_FLASH *nand_flash)
{
  ...
  if (nand_ctx->main_data_rx_cplt_semaphore.semaphore_handle == NULL)
  {
    nand_ctx->main_data_rx_cplt_semaphore.semaphore_handle = xSemaphoreCreateBinary();
    if (nand_ctx->main_data_rx_cplt_semaphore.semaphore_handle == NULL)
    {
      return LX_ERROR;
    }
  ...
  if (nand_ctx->main_data_tx_cplt_semaphore.semaphore_handle == NULL)
  {
    nand_ctx->main_data_tx_cplt_semaphore.semaphore_handle = xSemaphoreCreateBinary();
    if (nand_ctx->main_data_tx_cplt_semaphore.semaphore_handle == NULL)
    {
      return LX_ERROR;
    }
  ...
  if (nand_ctx->spare_data_rx_cplt_semaphore.semaphore_handle == NULL)
  {
    nand_ctx->spare_data_rx_cplt_semaphore.semaphore_handle = xSemaphoreCreateBinary();
    if (nand_ctx->spare_data_rx_cplt_semaphore.semaphore_handle == NULL)
    {
      return LX_ERROR;
    }
  ...
  if (nand_ctx->spare_data_tx_cplt_semaphore.semaphore_handle == NULL)
  {
    nand_ctx->spare_data_tx_cplt_semaphore.semaphore_handle = xSemaphoreCreateBinary();
    if (nand_ctx->spare_data_tx_cplt_semaphore.semaphore_handle == NULL)
    {
      return LX_ERROR;
    }
  ...
  w25n01gvxx_register_read_cplt_callback(p_w25n01gvxx_obj, w25n01gvxx_read_cplt_callback,
                                         (void *)&nand_ctx->main_data_rx_cplt_semaphore);
  w25n01gvxx_register_write_cplt_callback(p_w25n01gvxx_obj, w25n01gvxx_write_cplt_callback,
                                          (void *)&nand_ctx->main_data_tx_cplt_semaphore);
  w25n01gvxx_register_spare_read_cplt_callback(p_w25n01gvxx_obj, w25n01gvxx_spare_read_cplt_callback,
                                         (void *)&nand_ctx->spare_data_rx_cplt_semaphore);
  w25n01gvxx_register_spare_write_cplt_callback(p_w25n01gvxx_obj, w25n01gvxx_spare_write_cplt_callback,
                                          (void *)&nand_ctx->spare_data_tx_cplt_semaphore);
                                                  ...
                                                  UCHAR* main_buffer, UCHAR* spare_buffer, ULONG pages)
{
  ...
  if (main_buffer != NULL)
  {
    status = w25n01gvxx_read_dma(p_w25n01gvxx_obj, (uint16_t)block, (uint8_t)page, (uint8_t *)main_buffer, W25N01GVXX_DATA_PAGE_SIZE);
    if (status != W25N01GVXX_OK)
    {
      return status;
    }

    status = xSemaphoreTake(nand_ctx->main_data_rx_cplt_semaphore.semaphore_handle, pdMS_TO_TICKS(nand_ctx->nand_flash_op_timeout));
    if (status != pdTRUE)
    {
      return LX_ERROR;
    }
                                                   ...
                                                   UCHAR* main_buffer, UCHAR* spare_buffer, ULONG pages)
{
  ...
  status = w25n01gvxx_write_dma(p_w25n01gvxx_obj, block, page, (uint8_t *)main_buffer, W25N01GVXX_DATA_PAGE_SIZE);
  if (status != W25N01GVXX_OK)
  {
    return LX_ERROR;
  }
  status = xSemaphoreTake(nand_ctx->main_data_tx_cplt_semaphore.semaphore_handle, pdMS_TO_TICKS(nand_ctx->nand_flash_op_timeout));
  ...
  status = w25n01gvxx_write_spare_area_dma(p_w25n01gvxx_obj, block, page, 1, (uint8_t *)spare_buffer);
  if(status != W25N01GVXX_OK)
  {
    return LX_ERROR;
  }

  status = xSemaphoreTake(nand_ctx->spare_data_tx_cplt_semaphore.semaphore_handle, pdMS_TO_TICKS(nand_ctx->nand_flash_op_timeout));
...
/**
  ...
  * @param pobj w25n01gvxx_obj_t pointer
...
/**
  * @brief spare area read completion callback
  * @param pobj w25n01gvxx_obj_t pointer
  * @arg void user data pointer
  */
static void w25n01gvxx_spare_read_cplt_callback(w25n01gvxx_obj_t *pobj, void *arg)
...
/**
  * @brief spare area write completion callback
  * @param pobj w25n01gvxx_obj_t pointer
  * @arg void user data pointer
  */
static void w25n01gvxx_spare_write_cplt_callback(w25n01gvxx_obj_t *pobj, void *arg)
