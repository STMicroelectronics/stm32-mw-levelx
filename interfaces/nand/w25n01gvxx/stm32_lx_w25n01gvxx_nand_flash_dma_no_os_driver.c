...
UINT stm32_lx_w25n01gvxx_nand_flash_dma_no_os_driver_initialize(LX_NAND_FLASH *nand_flash)
{
                                                  ...
                                                  UCHAR* main_buffer, UCHAR* spare_buffer, ULONG pages)
{
  ...
  while (HAL_GetTick() - timeout_start < nand_ctx->nand_flash_op_timeout)
  {
    if (nand_ctx->main_data_rx_cplt == 1)
    {
      status = W25N01GVXX_OK;
      break;
    }
  }
  if (status != W25N01GVXX_OK)
  ...
  while (HAL_GetTick() - timeout_start < nand_ctx->nand_flash_op_timeout)
  {
    if (nand_ctx->spare_data_rx_cplt == 1)
    {
      status = W25N01GVXX_OK;
      break;
    }
                                                   ...
                                                   UCHAR* main_buffer, UCHAR* spare_buffer, ULONG pages)
{
  ...
  while (HAL_GetTick() - timeout_start < nand_ctx->nand_flash_op_timeout)
  {
    if (nand_ctx->main_data_tx_cplt == 1)
    {
      status = W25N01GVXX_OK;
      break;
    }
  ...
  while (HAL_GetTick() - timeout_start < nand_ctx->nand_flash_op_timeout)
  {
    if (nand_ctx->spare_data_tx_cplt == 1)
    {
      status = W25N01GVXX_OK;
      break;
    }
